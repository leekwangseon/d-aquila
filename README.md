...
이미 운영 중인 Prometheus가 `localhost:9090`에 있으면 D-aquila가 이를 사용합니다. 다른 주소를 사용하려면 환경 변수를 지정합니다.

```bash
export D_AQUILA_PROMETHEUS_URL=http://<prometheus-host>:9090
```

기존 Prometheus가 없으면 bundled Prometheus를 Docker Compose로 함께 실행할 수 있습니다.
